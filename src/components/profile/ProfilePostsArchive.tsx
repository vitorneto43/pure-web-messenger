...
import { MessageCircle, Heart, Pin, PinOff, Loader2, Image as ImageIcon, Video, Type } from "lucide-react";
...
export function ProfilePostsArchive({ userId, isOwner }: { userId: string; isOwner: boolean }) {
  ...
  async function load() {
    setLoading(true);
    const { data, error } = await (supabase as any).rpc("get_user_posts_archive", { _user_id: userId });
    if (error) {
      console.warn(error);
      setItems([]);
    } else {
  ...
  async function togglePin(id: string) {
    setBusy(id);
    const { error } = await (supabase as any).rpc("toggle_post_pin", { _post_id: id });
    setBusy(null);
  ...
  if (items.length === 0) {
    return (
      <div className="mt-6 text-center text-sm text-muted-foreground">
        {isOwner ? "Você ainda não criou nenhum post." : "Nenhum post ainda."}
      </div>
    );
  }
  ...
  return (
    <div className="mt-6">
      ...
      {rest.length > 0 && <Grid items={rest} isOwner={isOwner} busy={busy} togglePin={togglePin} />}
    </div>
  );
}
          ...
          >
            {it.kind === "image" && it.media_url ? (
              ...
              <div
                ...
                style={{ background: it.background ?? "linear-gradient(135deg,#6366f1,#ec4899)" }}
              >
                <span className="line-clamp-6">{it.content ?? it.caption ?? ""}</span>
              </div>
            )}

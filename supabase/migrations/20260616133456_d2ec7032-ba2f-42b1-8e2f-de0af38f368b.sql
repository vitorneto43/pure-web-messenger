  ...
  scored AS (
    SELECT a.*,
      (
        ...
        + (CASE WHEN cardinality(COALESCE(a.hashtags, '{}')) > 0 THEN 5 ELSE 0 END)
        + (random() * 5)
      )::numeric AS score
    FROM agg a
  )
...
GRANT EXECUTE ON FUNCTION public.discover_public_statuses(int, int) TO anon, authenticated, service_role;

CREATE OR REPLACE FUNCTION public.public_discover_people(_limit int DEFAULT 12)
RETURNS TABLE(id uuid, username text, display_name text, avatar_url text, city text, region text, country text, mutual_count int, reason text)
...
AS $$
  SELECT p.id, p.username, p.display_name, p.avatar_url,
         ...
         CASE
           WHEN EXISTS (SELECT 1 FROM public.statuses s WHERE s.user_id = p.id AND s.expires_at > now()) THEN 'publicou agora'
           ELSE 'novo no WaveChat'
         END AS reason
  ...
  ORDER BY
    CASE WHEN EXISTS (SELECT 1 FROM public.statuses s WHERE s.user_id = p.id AND s.expires_at > now()) THEN 0 ELSE 1 END,
    p.created_at DESC
  LIMIT LEAST(GREATEST(_limit, 1), 50);
$$;

GRANT EXECUTE ON FUNCTION public.public_discover_people(int) TO anon, authenticated, service_role;

CREATE OR REPLACE FUNCTION public.public_search_users(q text)
...
GRANT EXECUTE ON FUNCTION public.public_search_users(text) TO anon, authenticated, service_role;

CREATE OR REPLACE FUNCTION public.public_online_users(_limit int DEFAULT 30)
RETURNS TABLE(user_id uuid, username text, display_name text, avatar_url text, visibility text, online_at timestamptz)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $$
  SELECT p.id AS user_id, p.username, p.display_name, p.avatar_url, p.visibility, p.last_seen AS online_at
  FROM public.profiles p
  WHERE COALESCE(p.visibility, 'public') = 'public'
    AND p.banned_at IS NULL
...
GRANT EXECUTE ON FUNCTION public.public_online_users(int) TO anon, authenticated, service_role;